...
// POST - Simply mark order as complete (status change only)
export async function POST(
  ...
  try {
    ...
    // Simply update the order status to completed
    ...
    const updatedOrder = {
      ...
      notes: completionNotes || currentOrder.notes || `Order marked as complete by ${completedBy || 'Unknown'}`
    };
    ...
    console.error('Mark order complete error:', error);
    return NextResponse.json({
      success: false,
