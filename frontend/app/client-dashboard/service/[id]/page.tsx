...
export default function ServiceDetailPage() {
  ...
  const fetchServiceDetails = async () => {
    try {
      ...
      // Try to fetch from freelancer dashboard API by createdAt first
      let response;
      try {
        response = await fetch(`/api/freelancer-dashboard/service-by-createdat/${serviceId}`);
          ...
          const transformedService: Service = {
            ...
            additionalCharges: profile.additionalCharges ? [
              ...(profile.additionalCharges.fastDelivery ? [{
                name: 'Fast Delivery',
                price: profile.additionalCharges.fastDelivery.price?.toString() || '20'
              }] : []),
              ...(profile.additionalCharges.additionalChanges ? [{
                name: 'Additional Changes',
                price: profile.additionalCharges.additionalChanges.price?.toString() || '25'
              }] : [])
            ] : []
          };
        ...
        console.warn('Failed to fetch from freelancer dashboard API by createdAt, trying browse API:', apiError);
      }

      
      
      // Fallback: try to find by createdAt in browse API
      try {
        response = await fetch(`/api/freelancer-dashboard/browse?limit=100`);
        const result = await response.json();
        
        if (result.success && result.profiles) {
          // Find the specific service by createdAt (serviceId)
          const foundProfile = result.profiles.find(([email, profile]: [string, any]) => 
            profile.createdAt === serviceId
          );
        ...
        console.warn('Failed to fetch from freelancer dashboard browse API, trying services API:', apiError);
      }
